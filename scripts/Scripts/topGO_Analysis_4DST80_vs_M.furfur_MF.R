...
nodes_table = 30
nodes_graph = 5
table_outfile_SAM <- 'GeneTable_4DST80_vs_M.furfur_MF_SAM.txt'
table_outfile_Ebayes <- 'GeneTable_4DST80_vs_M.furfur_MF_Ebayes.txt'
graph_outfile_SAM <- 'Graph_4DST80_vs_M.furfur_MF_SAM.pdf'
graph_outfile_Ebayes <- 'Graph_4DST80_vs_M.furfur_MF_Ebayes.pdf'


file_geneID2GO <- annotation_file
geneID2GO <- readMappings(file=file_geneID2GO)
all_genesScores_SAM_df <- dbGetQuery(con,"SELECT name, p_value  FROM SAM_Results WHERE treatment = '4DST80' AND control= 'M.furfur'")
all_genesScores_Ebayes_df <- dbGetQuery(con,"SELECT name, adj_p_value  FROM Ebayes_Results WHERE treatment = '4DST80' AND control= 'M.furfur'") 
